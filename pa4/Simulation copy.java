...
//numjobs gets the amount of jobs in the file which is the first line/number
...
//		       System.out.println(array[i].toString());
	    ...
	    out1.println();
//this for loop places all jobs in the correct order in storage queues.
	    boolean empty = true;    
        boolean check = false;   
...
//          8.          complete all processes finishing now
            for(int p = 0; p < n; p++){
                if(array[p].isEmpty() != true){
                    if(((Job)array[p].peek()).getFinish() == time){
                        jobs[count++] = ((Job)array[p].peek()).getWaitTime();
                        swit.enqueue(((Job)array[p].dequeue()));
                        check = true;
                    }
                }
            }
...
//          Queue of minimum length and with lowest index in the queue array.
            while((!storage.isEmpty()) && ((Job)(storage.peek())).getArrival() == time){
                ...
                int shortest = 0;
                for(int q = 0; q < n; q++){
                    if(array[q].length() < min){
                        min = array[q].length();
                        shortest = q;
                    }
                }
                array[shortest].enqueue(storage.dequeue());
                check = true;
            }
//          10.     } end loop
            empty = true;
...
//if there is only one processor print the singular version of the word.
//if there is more than one print the plural version, as well as all the relative times.
          if(n == 1){
	          out.println("1 processor: totalWait=" + tw + ", maxWait=" + mw + ", averageWait=" + String.format("%.2f", avgw));
	       }else if (n >= 2){
	    	   out.println(n + " processors: totalWait=" + tw + ", maxWait=" + mw + ", averageWait=" + String.format("%.2f", avgw));
	       }
